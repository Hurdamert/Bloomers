...
6. Double-click `run_bloomer.bat`. It automatically finds `py`, `python`, or `python3` and installs Bloomer's small Windows OCR components on first launch. For a manual setup, run `python -m pip install -r requirements.txt` and then `python bloomer.py`.
...
Keyboard and mouse actions use Windows `SendInput`, so BTD6 receives tower hotkeys, upgrade hotkeys, and explicit left-click placement events reliably. Cash recognition runs locally through the OCR engine built into Windows; no screenshots or account information are uploaded.
...
- The selected base tower must be unlocked. Upgrade paths may be only partially unlocked; unavailable tiers are skipped automatically.
